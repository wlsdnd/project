function media_layer_open(id, top, left, src, track){
	...
	//console.log(track);
    try {
        var tempOrigin = $(document).find("#media_layer_contents_" + id);
        var temp = $(document).find("#media_layer_contents_" + id)[0];
        var ifrm = $(temp).find("iframe");

        //var ifrm = $(temp[0]).find("#mediapop");

        var ifrmContents = ifrm[0].contentWindow.document;

        //	var ifrmContents = $(ifrm).contentWindow;

        var mediasrc = ifrmContents.getElementById('mediasrc');
        $(mediasrc).attr("src", src);

        if (id.indexOf("type3")) {
            var trackObj = ifrmContents.getElementById('track');
            $(trackObj).attr("src", track);
        }

        var audio = ifrmContents.getElementById('video');

        //	alert(audio.outerHTML);
	...
	$("#media_layer_"+id).fadeIn();
		...
		if (pos.indexOf('d') >= 0) {
			// down
			y += $('#'+id).outerHeight();
			y += ((r-1)*ifrm.outerHeight());
		} else {
			// up
			y -= (r*ifrm.outerHeight());
		}

		if (y <= 30) {	// y(top) 값이 너무 작거나 크면 화면을 벗어날 수 있으므로
			y = 30;
		} else if (y + ifrm.outerHeight() >= viewportSize.height - 30) {
			y = viewportSize.height - 30 - ifrm.outerHeight();
		}
		...
		if (x + ifrm.outerWidth() >= viewportSize.width - 30)
		{
			$("#media_layer_"+id).css('left', (viewportSize.width - ifrm.outerWidth() - 30) + 'px');
		} else {
			$("#media_layer_"+id).css('left', x + 'px');
		}
		// iframe 내부의 미디어 플레이어 위치
		if (ifrm.outerWidth() < $(document).width()) tempOrigin.css('margin-left', '-'+ifrm.outerWidth()/2+'px');
		else ifrm.css('left', '0px');

		$("#media_layer_"+id).width((ifrm.outerWidth()+30));	// 너비 지정
		$("#media_layer_"+id).height(74);	// 높이 강제 지정
		tempOrigin.css('top', '0px');
		tempOrigin.css('left', (ifrm.outerWidth()/2+25)+'px');
...
function preLoadIfram(id){	
	...
	var iFrame = document.createElement('iframe');
        ...
        //위 data(type)에 2번이 적용되어야 하는데 이게 제대로 안된 컨텐츠가 있어 강제로 media2.html 적용해주기 위함.
    //기본 버튼 나오는 형태임
    /*
		if (id.indexOf("type4") >= 0) {
		    iFrame.src = "pp_overlay/media/media4.html";
		}*/

		temp.appendChild(iFrame);	// 미디어 플레이어 추가 및 로딩 완료시 열기. type 1 의 경우 이동 아이콘 숨김 처리.

		//alert(JSON.stringify(temp));
}

function image_layer_open2(id, status){	// 이미지 레이어 표시
	...
	if ($('#image_layer_contents_'+id).length > 0)	// 현재 화면에 image layer가 존재하는 경우
	{
		if(!status){
			...
			$('#image_layer_'+id).fadeIn();
			//$('#image_layer_'+id).css('display', 'block');
		}
	} else {										// 존재하지 않는 경우
		...
		if(!status){
			...
			gfs.contents().find('#image_layer_'+id).fadeIn();
		}
	}
}
...
function stop_sound_all() {		// 현재 모든 sound player를 찾아서 pause 시키는 function
	...
	for (var i=0; i<gfs.length; i++)
	{
	    ...
	    var ifrm = $(gfs[i].contentWindow.document).find("iframe");

	    //var ifrm = $(temp[0]).find("#mediapop");

	    for (var j = 0; j < ifrm.length; j++) {

	        var ifrmContents = ifrm[j].contentWindow.document;

	        var audio = ifrmContents.getElementById('video');

	        allSoundArr.push(audio);
	    }
	...
	{
	    allSoundArr[i].pause();	// puase
	    allSoundArr[i].currentTime = 0;
	}
}
...
function play_sound_this(id) {	// 현재 페이지에 있는 sound player만 play 
...
$( document ).ready(function() {
	...
	$("button").each(function(){
		//alret($(this).attr('id'));
		if($(this).attr('class') == "type1" || $(this).attr('class') == "type2" || $(this).attr('class') == "type3")
		{
...
//		media_layer_open("../assets/video/"+event.target.id+".mp4");return false;
    ...
    $(".type1").off('click').on('click', function (e) {	// type1 버튼 (투명)
		...
		else if ($(this).hasClass('drawer'))	// 페이지 이동 버튼인 경우
		{
			showDrawing();
		}
		else								// 그 외에는 모두 음원 버튼
	...
	$(".drawer").off('click').on('click', function (e) {	// type1 버튼 (투명)
		showDrawing();
	});
    ...
    $(".type3").off('click').on('click', function (e) {	// type3 음원 버튼
		mediaBtn(e);
		$(this).next('.audio_btn_wrap').css('display','block');
	});
	/* ------- 2018-11-06 김영진 수정 ------- */
...
function mediaBtn(event) {
	...
	for (var i=0;i<document.getElementsByName("sound").length ; i++) {
		if (document.getElementsByName("sound")[i].parentElement.id===("media_layer_contents_"+event.target.id)) {
		    if (document.getElementsByName("sound")[i].src.indexOf("media1") >= 0) {
				...
				if (event.target.id.indexOf("type1") >= 0) {
	...
	if ($('#media_layer_'+event.target.id).css('display')=="block") {
		...
		$('#'+event.target.id+'_stop').css('display','block').click(function(){
			stop_sound_this();
			$(this).parent().css('display','none');
		});
		$('#'+event.target.id+'_pause').css('display','block').click(function(){
			$(this).css('display','none').next().css('display','block');
			stop_sound_this();
		});	
		$('#'+event.target.id+'_play').click(function(){
			$(this).css('display','none').prev().css('display','block');
			play_sound_this(event.target.id);
		}); 
		/* ------- 2018-11-06 김영진 수정 ------- */

		/*
		$('#'+event.target.id+'_stop').click(function(){
			stop_sound_this();
			$('.play_btn_wrap').css('display','none');
		});
		$('#'+event.target.id+'_pause').click(function(){
			$(this).toggleClass('on');
			stop_sound_this();
		});
		*/
		...
		var tempOrigin = $(document).find("#media_layer_contents_" + event.target.id);
		var temp = $(document).find("#media_layer_contents_" + event.target.id)[0];
		var ifrm = $(temp).find("iframe");
		...
		audio.addEventListener("ended", function () {
		    ...
		    $('#' + event.target.id + '_pause').css('display', 'none').next().css('display', 'block');
		...
		if (event.target.id.indexOf("type1") >= 0 || event.target.id.indexOf("type4") >= 0) {
		    $('#media_layer_' + event.target.id).css('display', 'none');
		}
...
function showDrawing() {	
	window.parent.viewer.showDrawing();
}

/* ------- 2018-11-07 김영진 수정 ------- */
$(document).ready(function() {
	$('#popClose').click(function(){
		$('.image_layer').css('display','none');
	});
});
/* ------- 2018-11-07 김영진 수정 ------- */